  ...
  try {
    ...
    if (method === 'tools/call') {
      ...
      if (name === 'create_task') {
        ...
        // Auto-lookup project ID from name if provided
      ...
      // SMART ADD TASKS
      if (name === 'smart_add_tasks') {
        // Get all projects for categorization context
        const projectsRes = await fetch(`${TODOIST_API}/projects`, {
          headers: { 'Authorization': `Bearer ${TODOIST_TOKEN}` }
        });
...
export async function GET() {
  return Response.json({ 
    status: 'ok',
    message: 'Todoist MCP Server - Enhanced Edition',
    features: [
      'Full task creation with descriptions, recurring schedules, labels',
      'Batch task creation',
